...
FAQ_Details <- list(
  ...
  h2(tags$b("What is a demand charge?")),
  h3("A demand charge depends on the combination of the demand charge price (see below) multiplied by the peak power usage. In our calculations, we assume that demand charge will be assessed based on the maximum power usage by the lighting system."),
  h2(tags$b("What is a demand charge price?")),
  h3("Commercial customers typically pay a demand charge as part of the elecricity bill, based on their peak power usage. The price for this charge is normally shown on the power bill as '$/kW'."),
  h2(tags$b("What is DLI capacity?")),
  h3("The total amount of light that can be provided by the lighting system in a day. Calculated from the PPFD capacity (see above) and photoperiod."),
